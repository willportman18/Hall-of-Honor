...
<head>
<meta charset="UTF-8">
<title>Cardinal Bellarmine Award</title>
 ...
 <a href="home.html" class="back"><i class="fa fa-angle-left fa-2x" aria-hidden="true"></i></a>
  <?php
    ...
    $password = $ini_array['password'];
	  ...
	  if ($pdo)
	  {
	    $smt = $pdo->prepare("SELECT * FROM cardinalBellarmineAward ORDER BY `Class of`");
		if ($smt->execute())
		{
		  $rows = $smt->fetchAll();
		  if (count($rows)==0){
			...
			foreach($rows as $row)
			{
			  echo '<h1 class="name">Name: '.$row['Name'].'</h1>
<br>';
			  echo '<h1
 class="class">Class: '.$row['Class of'].'</h1><br><br>';
			}
		  }
		}
	  }
